import numpy as np
import pypsg.cfg as cfg
from pypsg.run_psg import run_psg
from pypsg.utils import transform_date
import pypsg.atm_obj as atm

cfg_path = '/home/buriola/PSG/PSG4Mars/data/cfg/test_rad/'
cfg_b = cfg.read_cfg(f'{cfg_path}basic_cfg.txt')

list_lat = [-45, 0, 45]
list_long = [0, 180]
date = '2019/12/23 00:00'

rad_path = '/home/buriola/PSG/PSG4Mars/data/rad/'
ranges = np.arange(100,2150,50)

for lat in list_lat:
    for long in list_long:

        stringfile = f"{'{:.0f}'.format(lat)}_{'{:.0f}'.format(long)}_{transform_date(date)}"
        cfg.generate_profile(cfg_b, date, lat, long, cfg_path)

        cfg_ds = cfg.read_cfg(f'{cfg_path}cfg_{stringfile}.txt')
        atmos = atm.atmosphere()
        atmos.get_atmosphere(cfg_ds)

        atmos.continuum_list = ['Refraction', 'CIA_all']
        atmos.aerosol_list = []

        atmos.edit_cfg(cfg_ds)

        cfg_ds['SURFACE-EMISSIVITY'] = 1.
        cfg_ds['SURFACE-ALBEDO'] = 0.


        for i in range(len(ranges)-1):
            cfg_ds['GENERATOR-RANGE1'] = ranges[i]
            cfg_ds['GENERATOR-RANGE2'] = ranges[i+1]
            cfg.dict_to_cfg(cfg_ds, f'{cfg_path}cfg_{stringfile}_hr.txt')
            run_psg(cfg_file=f'{cfg_path}cfg_{stringfile}_hr.txt', type='rad', out_file=f"{rad_path}rad_{stringfile}_freq{ranges[i]}_{ranges[i+1]}_hr.txt", verbose=False)